...
export function Hero({ profile }: HeroProps) {
  const contactItems = [
    ...
    },
    {
      icon: Github,
      label: "GitHub",
      value: profile.contact.github ? profile.contact.github.replace('https://', '') : undefined,
    ...
    },
    {
      icon: Linkedin,
      label: "LinkedIn",
      value: profile.contact.linkedin ? profile.contact.linkedin.replace('https://', '') : undefined,
    ...
    },
    {
      icon: Twitter,
      label: "Twitter",
      value: profile.contact.twitter ? profile.contact.twitter.replace('https://', '') : undefined,
    ...
    },
    {
      icon: Instagram,
      label: "Instagram",
      value: profile.contact.instagram ? profile.contact.instagram.replace('https://', '') : undefined,
  ...
  return (
    <section className="max-w-2xl mx-auto px-6 pt-12 pb-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-geist-xl font-medium text-white mb-1">{profile.name}</h1>
          <p className="text-geist-sm text-gray-500">{profile.tagline}</p>
        </div>
        <HamburgerMenu>
          <div className="space-y-4">
            <div>
              <h3 className="text-geist-base font-medium text-white mb-3">Contato Rápido</h3>
              <div className="space-y-2">
                {contactItems.map((item) => (
                  ...
                  >
                    <div className={`p-1.5 rounded ${item.color} bg-gray-800/30`}>
            ...
            <div className="pt-4 border-t border-gray-800">
              <h3 className="text-geist-base font-medium text-white mb-3">Localização</h3>
              <p className="text-geist-sm text-gray-400">
                {profile.location.city}, {profile.location.state}
              </p>
              <p className="text-xs text-gray-500 mt-1">
                {profile.location.availableForRemote ? "Disponível para trabalho remoto" : "Trabalho local"}
              </p>
            </div>
          </div>
        </HamburgerMenu>
      ...
      <div className="mb-8">
        <h2 className="text-geist-lg font-medium mb-4 text-white leading-tight">
          ...
          <em className="italic font-medium">tecnologias modernas</em> e código limpo.
        </h2>
      </div>
    </section>
  )
